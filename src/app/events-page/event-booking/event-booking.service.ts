...
export class EventBookingService {
   ...
   * Validate family
   */
  validateFamily(memberNumber, eventId, eventday) {
    let validateFamilyURL = ServerApis.validateFamily;
    validateFamilyURL = validateFamilyURL.replace('memberno', memberNumber).replace('eventId', eventId).replace('eventDay', eventday);
